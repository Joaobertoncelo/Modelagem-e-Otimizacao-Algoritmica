...
def ant_system_corte(num_formigas, num_iteracoes, alpha, beta, rho, Q, itens, capacidade):
    ...
    # execute a função para obter a melhor solução e o melhor valor
    melhor_solucao, melhor_valor = ant_system_corte(num_formigas, num_iteracoes, alpha, beta, rho, Q, itens, capacidade)

    # plota um gráfico de barras que mostra a quantidade de cada item que foi selecionado na solução encontrada 
    plt.bar(range(len(melhor_solucao)), melhor_solucao)
    plt.xlabel('Item')
...
if __name__ == '__main__':
    ...
    capacidade = 10
    ant_system_corte(num_formigas, num_iteracoes, alpha, beta, rho, Q, itens, capacidade)
